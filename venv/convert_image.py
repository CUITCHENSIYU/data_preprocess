...
INPUT_DATA='smear_convert_size'
OUTPUT_DATA='processed_smear_data_rgb/smear_dataset.npy'
...
def create_image_lists(sess, testing_percentage):
    ...
    current_label = 1 # 在接下来的for循环中，第一次循环时值为0，每次循环结束时加一
    count = 1
    ...
    for sub_dir in sub_dirs:
        ...
        #获取一个子目录中的所有图片数据
        extensions = ['jpg', 'jpeg', 'JPEG','png']#列出所有扩展名
        file_list=[]
        # 返回子文件夹的名称（sub_dir是包含文件夹地址的串，去掉其地址，只保留文件夹名称）
        ...
        for file_name in file_list:
            ...
            # image = tf.image.resize_images(image, [72, 72],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            ...
            #         image_value)  # 由于一共有3670张图片，这样最终的validation_images的尺寸大致是(3670*validation_percentage%)*229*229*3
            #     validation_labels.append(
            #         current_label)  # 由于一共有3670张图片，这样最终的validation_labels的尺寸大致是(3670*validation_percentage%)*1
